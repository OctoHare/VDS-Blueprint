...
- придумайте свой логин для администратора, оставлять `admin` слишком очевидно 
- придумайте свой сложный пароль для администратора
- подтвердите свой пароль, введите его второй раз
- введите **setup token**
...
EOF
```
> ℹ️ В дальнейшем эту страницу можно легко заменить на свою. Перейдите в бесплатный чат с ИИ [DeepSeek](https://chat.deepseek.com) (или любой привычный и удобный для вас ИИ инструмент) и попросите его что-то вроде - `Сделай одностраничный сайт. На тему экологической проблематики кишечных газов крупного рогатого скота, где подробно рассматривается эта проблема, последствия и варианты решения. Сайт не должен содержать ссылок или кнопок, это одностраничный сайт заглушка и не должен содержать не рабочие элементы. Дизайн, заголовок и favicon должны соответствовать выбранной тематике сайта`. Далее полученное содержимое можно просто целиком перенести в ваш `index.html`.
...
```
<br><br>
❗Внимание, данный ⬆️ блок ⬆️ команд ⬆️ можно спокойно использовать для обновления версии Portainer.
...
> ✔️ На данном этапе базовая инфраструктура сервера полностью развёрнута. Веб-сервер Caddy взял на себя всю работу по взаимодействию с внешним миром: он автоматически поддерживает шифрование трафика (занимается выпуском и перевыпуском SSL-сертификатов) и выдаёт браузерам заголовки безопасности. Графический веб-интерфейс Portainer теперь локально изолирован и доступен из глобальной сети только по заданному секретному пути. Сервер полностью готов к эксплуатации!
<br>
